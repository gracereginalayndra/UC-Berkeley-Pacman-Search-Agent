...
def runExtraction(args):
    ...
    featureFunction = lambda gameState: FeatureExtractor.extractFeatureForAllActions(gameState)
    ...
    df_train = pd.DataFrame(data=trainingData, columns=FEATURE_NAMES)
    df_train["label"] = trainingLabels
    df_train.to_csv("./pacmandata/q3_train.csv", index=False)

    df_validation = pd.DataFrame(data=validationData, columns=FEATURE_NAMES)
    df_validation["label"] = validationLabels
    df_validation.to_csv("./pacmandata/q3_validation.csv", index=False)

    df_test = pd.DataFrame(data=testData, columns=FEATURE_NAMES)
